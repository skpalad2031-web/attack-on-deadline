"""
Project Title: Attack On Deadline
Group Members: Sofia Palad, Naiah Intong, Isha Shekar
Description: A task reminder system for PSHS students.
"""

# Import datetime module for date calculations
from datetime import datetime

def main():
    # Get current date from user
    cd = input("Enter current date (YYYY-MM-DD): ")
    
    # Setup lists to store data
    task_names = []
    task_deadlines = []

    print("--- Welcome to Attack On Deadline ---")

    # Input Implementation
    while True:
        # Get task details from user
        name = input("Enter task name: ")
        date = input("Enter deadline (YYYY-MM-DD): ")

        # Store task data into lists
        task_names.append(name)
        task_deadlines.append(date)

        # Loop control
        choice = input("Add another task? (y/n): ")
        if choice.lower() != 'y':
            break

    print("\nTasks loaded. System ready.")

    # Display days left for each task
    for i in range(len(task_names)):
        # Calculate days left by subtracting current date from deadline
        days_left = (datetime.strptime(task_deadlines[i], "%Y-%m-%d") - datetime.strptime(cd, "%Y-%m-%d")).days
        print(f"Task: {task_names[i]} | Days you have left: {days_left}")

if __name__ == "__main__":
    main()
